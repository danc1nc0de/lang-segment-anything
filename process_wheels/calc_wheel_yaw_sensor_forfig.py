...
def get_wheel_direction_sensor(wheel_ground_point, calibrated_sensor_data):
    ...
    wheel_direction_ego = Kinv @ ((kinv.T @ P_i_1) * P_i_0 - (kinv.T @ P_i_0) * P_i_1)
    return wheel_direction_ego


def stat_angle_noise_for_wheels(delta_img_corners_dict, boxes_3d_sensor, calibrated_sensor_data, img_name,
                                cam_sensor):
    if img_name not in delta_img_corners_dict:
        delta_img_corners_dict[img_name] = {}
    for box_3d_sensor in boxes_3d_sensor:
        if not box_3d_sensor.wheel_direction_valid:
            continue
        if box_3d_sensor.token not in delta_img_corners_dict[img_name]:
            delta_img_corners_dict[img_name][box_3d_sensor.token] = []
        for u_noise in PIXEL_NOISE_LIST:
            for v_noise in PIXEL_NOISE_LIST:
                wheel_ground_point_noise = [
                    box_3d_sensor.wheel_ground_point[0] + u_noise,
                    box_3d_sensor.wheel_ground_point[1] + v_noise,
                    box_3d_sensor.wheel_ground_point[2],
                    box_3d_sensor.wheel_ground_point[3]
                ]
                wheel_direction_sensor_noise = get_wheel_direction_sensor(wheel_ground_point_noise,
                                                                          calibrated_sensor_data)
                wheel_yaw_noise = np.arctan2(wheel_direction_sensor_noise[2], wheel_direction_sensor_noise[0])
                delta_wheel_yaw = wheel_yaw_noise - box_3d_sensor.wheel_yaw
                delta_img_corners_dict[img_name][box_3d_sensor.token].append(
                    [np.rad2deg(delta_wheel_yaw), [int(u_noise), int(v_noise)]])


def add_angle_noise(box_3d_sensor, angle_noise_deg, angle_noise_type):
    box_3d_sensor_noise = box_3d_sensor.copy()
    angle_noise = np.deg2rad(angle_noise_deg)
    yaw_sensor, pitch_sensor, roll_sensor = R.from_matrix(box_3d_sensor.rotation_matrix).as_euler('zyx', degrees=False)
    quat_sensor = R.from_euler('zyx', [yaw_sensor, pitch_sensor, roll_sensor], degrees=False).as_quat(scalar_first=True)
    if angle_noise_type == 'yaw':
        quat_sensor_noise = R.from_euler('zyx', [yaw_sensor + angle_noise, pitch_sensor, roll_sensor],
                                         degrees=False).as_quat(
            scalar_first=True)
    elif angle_noise_type == 'pitch':
        quat_sensor_noise = R.from_euler('zyx', [yaw_sensor, pitch_sensor + angle_noise, roll_sensor],
                                         degrees=False).as_quat(
            scalar_first=True)
    elif angle_noise_type == 'roll':
        quat_sensor_noise = R.from_euler('zyx', [yaw_sensor, pitch_sensor, roll_sensor + angle_noise],
                                         degrees=False).as_quat(
            scalar_first=True)
    else:
        assert False
    [x, y, z] = box_3d_sensor.center
    box_3d_sensor_noise.translate(-np.array([x, y, z]))
    box_3d_sensor_noise.rotate(Quaternion(quat_sensor).inverse)
    box_3d_sensor_noise.rotate(Quaternion(quat_sensor_noise))
    box_3d_sensor_noise.translate(np.array([x, y, z]))
    return box_3d_sensor_noise
...
def stat_angle_noise_for_boxes(delta_img_corners_dict, boxes_3d_sensor, calibrated_sensor_data, img_name, cam_sensor):
    cam_intrinsic = np.array(calibrated_sensor_data['camera_intrinsic'])
    if img_name not in delta_img_corners_dict:
        delta_img_corners_dict[img_name] = {}
    for box_3d_sensor in boxes_3d_sensor:
        ...
        for angle_noise_deg in ANGLE_NOISE_DEG_LIST:
            box_3d_sensor_noise = add_angle_noise(box_3d_sensor, angle_noise_deg, 'yaw')
            img_corners_noise = view_points(box_3d_sensor_noise.corners(), cam_intrinsic, normalize=True)[:2, :]
...
def update_wheel_direction_dict(wheel_direction_dict, boxes_3d_sensor, img_name, cam_sensor):
    if img_name not in wheel_direction_dict:
        wheel_direction_dict[img_name] = {}
    for box_3d_sensor in boxes_3d_sensor:
        if box_3d_sensor.wheel_direction_valid:
            if box_3d_sensor.token not in wheel_direction_dict[img_name]:
                wheel_direction_dict[img_name][box_3d_sensor.token] = {}
            wheel_direction_dict[img_name][box_3d_sensor.token]['wheel_direction'] = \
                box_3d_sensor.wheel_direction.tolist()
...
def draw_wheel_direction(img, boxes_3d_sensor, calibrated_sensor_data, color=COLOR_GREEN):
    ...
    for idx, box_sensor in enumerate(boxes_3d_sensor):
        if idx != 0:
            continue
...
def update_box_wheel_direction(wheel_direction_sensor, wheel_yaw_sensor, box_sensor, P_i_0, P_i_1,
                               wheel_token_0, wheel_token_1, thr=np.radians(45.0)):
    wheel_yaw_valid = 0
    box_sensor_yaw, _, _ = R.from_matrix(box_sensor.rotation_matrix).as_euler('zyx', degrees=False)
    if np.abs(wheel_yaw_sensor - box_sensor_yaw) < thr:
        wheel_yaw_valid = 1
        # wheel_direction_sensor = wheel_direction_sensor
        # wheel_yaw_sensor = wheel_yaw_sensor
        # P_i_0, P_i_1 = P_i_0, P_i_1
        # wheel_token_0, wheel_token_1 = wheel_token_0, wheel_token_1
    elif np.abs(wheel_yaw_sensor + np.pi - box_sensor_yaw) < thr:
        wheel_yaw_valid = 1
        wheel_direction_sensor = -wheel_direction_sensor
        wheel_yaw_sensor = wheel_yaw_sensor + np.pi
        ...
        P_i_0, P_i_1 = P_i_1, P_i_0
        wheel_token_0, wheel_token_1 = wheel_token_1, wheel_token_0
    elif np.abs(wheel_yaw_sensor + 2. * np.pi - box_sensor_yaw) < thr:
        wheel_yaw_valid = 1
        # wheel_direction_sensor = wheel_direction_sensor
        wheel_yaw_sensor = wheel_yaw_sensor + 2. * np.pi
        # P_i_0, P_i_1 = P_i_0, P_i_1
        # wheel_token_0, wheel_token_1 = wheel_token_0, wheel_token_1
    elif np.abs(wheel_yaw_sensor - 2. * np.pi - box_sensor_yaw) < thr:
        wheel_yaw_valid = 1
        ...
        wheel_yaw_valid = 0
    yaw_diff = np.abs(wheel_yaw_sensor - box_sensor_yaw)
    if wheel_yaw_valid > 0:
        # already have a valid wheel direction
        if box_sensor.wheel_direction_valid:
            if yaw_diff < np.abs(box_sensor.wheel_yaw - box_sensor_yaw):
                box_sensor.wheel_direction = wheel_direction_sensor
                box_sensor.wheel_yaw = wheel_yaw_sensor
                box_sensor.wheel_ground_point = [P_i_0[0], P_i_0[1], P_i_1[0], P_i_1[1]]
                box_sensor.wheel_token = [wheel_token_0, wheel_token_1]
        else:
            box_sensor.wheel_direction_valid = True
            box_sensor.wheel_direction = wheel_direction_sensor
            box_sensor.wheel_yaw = wheel_yaw_sensor
            box_sensor.wheel_ground_point = [P_i_0[0], P_i_0[1], P_i_1[0], P_i_1[1]]
            box_sensor.wheel_token = [wheel_token_0, wheel_token_1]
...
def update_wheel_direction(boxes_sensor, wheel_annos, sensor_name, calibrated_sensor_data):
    # calc calibration
    K = np.array(calibrated_sensor_data['camera_intrinsic'])
    Kinv = np.linalg.inv(K)
    kinv = Kinv[1, :]
    for box_sensor in boxes_sensor:
        idxes_wheel = [idx for idx, token in enumerate(wheel_annos['assoc_box_tokens']) if token == box_sensor.token]
        for i in range(len(idxes_wheel)):
            ...
            for j in range(i + 1, len(idxes_wheel)):
                ...
                P_i_1 = np.array([u_grd_1, v_grd_1, 1.0])
                wheel_direction_sensor = Kinv @ ((kinv.T @ P_i_1) * P_i_0 - (kinv.T @ P_i_0) * P_i_1)
                wheel_yaw_sensor = np.arctan2(wheel_direction_sensor[2], wheel_direction_sensor[0])
                update_box_wheel_direction(wheel_direction_sensor, wheel_yaw_sensor, box_sensor,
...
def filtering_not_valid_boxes_for_calc_yaw(boxes_sensor, wheel_annos):
    boxes_sensor_output = []
    for box_sensor in boxes_sensor:
        # filtering no wheels
        if box_sensor.token not in wheel_annos['assoc_box_tokens']:
            continue
    ...
    for cam_sensor in tqdm(CAM_SENSORS, desc="cam_sensor"):
        ...
        for scene in tqdm(nusc.scene, desc="scene"):
            ...
            for sample_token in tqdm(sample_token_lst, desc="sample_token"):
                ...
                img_name = sample_sensor_data['filename'].split('/')[-1].split('.')[0]
                if img_name != 'n008-2018-08-27-11-48-51-0400__CAM_BACK_LEFT__1535385094147405':
                    continue
                ...
                boxes_3d_sensor_filtering = filtering_not_valid_boxes_for_calc_yaw(boxes_3d_sensor_veh,
                                                                                   annos_wheel[img_name])
                update_wheel_direction(boxes_3d_sensor_filtering, annos_wheel[img_name], cam_sensor,
                                       calibrated_sensor_data)
                boxes_3d_sensor_filtering = filtering_not_wheel_direction_valid_boxes(boxes_3d_sensor_filtering)
                update_wheel_direction_dict(wheel_direction_dict, boxes_3d_sensor_filtering, img_name, cam_sensor)

                stat_angle_noise_for_boxes(delta_box_img_corners_dict, boxes_3d_sensor_filtering,
                                           calibrated_sensor_data,
                                           img_name, cam_sensor)
                stat_angle_noise_for_wheels(delta_wheel_img_corners_dict, boxes_3d_sensor_filtering,
                                            calibrated_sensor_data, img_name, cam_sensor)
                colors_map = get_colors_map(len(boxes_3d_sensor_veh))
                ...
                # scale = 1.5
                # vmin, vmax, umin, umax = 460, 850, 150, 750
                # img_notxt = np.asarray(img_notxt).copy()
                # img_crop = img_notxt[vmin:vmax, umin:umax]
                # img_crop = Image.fromarray(np.uint8(img_crop)).convert("RGB")
                # original_size = img_crop.size  # 获取原始尺寸 (width, height)
                # new_size = (int(original_size[0] * scale), int(original_size[1] * scale))  # 计算新尺寸
                # img_crop = img_crop.resize(new_size, Image.LANCZOS)
                # img_crop = np.asarray(img_crop).copy()
                # img_notxt[0:new_size[1], (1600-new_size[0]):1600] = img_crop
                # # img_notxt[(900 - new_size[1]):900, 0:new_size[0]] = img_crop
                # # img_notxt[0:new_size[1], 0:new_size[0]] = img_crop
                # img_notxt = Image.fromarray(np.uint8(img_notxt)).convert("RGB")
